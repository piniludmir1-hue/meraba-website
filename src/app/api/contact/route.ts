...
function buildRows(inquiry: ContactRequest, labels: Array<{ key: keyof ContactRequest; label: string }>) {
  return labels
    .map((field) => {
      ...
      return `<tr><th align="left" style="width:42%;padding:11px 14px;border-bottom:1px solid #e6ebf0;color:#2d3a46;font-size:13px;font-weight:700;line-height:1.4;">${escapeHtml(field.label)}</th><td style="padding:11px 14px;border-bottom:1px solid #e6ebf0;color:#07111f;font-size:13px;line-height:1.55;">${textToHtml(value)}</td></tr>`
    })
    .join('')
}
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;border-collapse:collapse;background:#f4f7fa;">
      <tr>
        <td align="center" style="padding:32px 16px;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;max-width:640px;border-collapse:collapse;background:#ffffff;border:1px solid #dbe3eb;">
            <tr>
              <td align="center" style="padding:38px 34px 22px 34px;">
            ...
            <tr>
              <td style="padding:0 34px 28px 34px;text-align:center;">
                <h1 style="margin:0;color:#07111f;font-size:28px;line-height:1.15;font-weight:600;letter-spacing:-0.02em;">${escapeHtml(autoresponder.heading)}</h1>
                <p style="margin:18px auto 0 auto;max-width:470px;color:#4d5b69;font-size:15px;line-height:1.7;">${textToHtml(autoresponder.introText)}</p>
              </td>
            </tr>
            <tr>
              <td style="padding:0 34px 14px 34px;">
                <div style="height:3px;width:54px;background:#083f68;margin:0 auto 24px auto;"></div>
            ...
            <tr>
              <td style="padding:0 34px 38px 34px;text-align:center;">
                <p style="margin:0;color:#4d5b69;font-size:14px;line-height:1.7;">${textToHtml(autoresponder.closingText)}</p>
                <p style="margin:22px 0 0 0;color:#07111f;font-size:15px;font-weight:700;letter-spacing:0.08em;">${escapeHtml(autoresponder.signatureCompanyName)}</p>
                <p style="margin:8px 0 0 0;color:#083f68;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;">${escapeHtml(autoresponder.brandTagline)}</p>
              </td>
            </tr>
          </table>
        </td>
  ...
  const confirmationResponse = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      text: [
        ...
        text,
        '',
        contactContent.autoresponder.closingText,
        '',
        contactContent.autoresponder.signatureCompanyName,
